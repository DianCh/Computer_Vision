...
buffer = cell(2 * (nr + nc) + 2, 1);

% Create a white canvas
canvas = uint8(ones(size(I, 1) + nr, size(I, 2) + nc, 3) * 255);
...
for i = 1 : nr
    ...
    [repository{i + 1, 1}, E] = insHorSeam(repository{i, 1}, My, Tby);
...
for j = 1 : nc
    ...
    [repository{1, j + 1}, E] = insVerSeam(repository{1, j}, Mx, Tbx);
...
if generateAVideo
    ...
    while ~(tracer_i == 1 && tracer_j == 1)
        inserted = repository{tracer_i, tracer_j};
        indicated = indicators{tracer_i, tracer_j};

        insertedN = size(inserted, 1);
        insertedM = size(inserted, 2);
        indicatedN = size(indicated, 1);
        indicatedM = size(indicated, 2);

        canvas(1:insertedN, 1:insertedM, :) = inserted;
        buffer{k, 1} = canvas;
